...
class Solution:
    ...
    def findSpecialInteger(self, arr: List[int]) -> int:
        ...
        """
        # we can set the value of the minimum occurence min
        min = len(arr) // 4 + 1
        # then start a loop through the array storing the
        # current value curr and the count(number of occurrences) of each value
        i = 0
        while i < len(arr):
            cnt = 0
            curr = arr[i]
            while i < len(arr) and arr[i] == curr:
                cnt += 1
                i += 1
            # finally check whether the count is greater than or
            # equal to min
            if cnt >= min:
                return curr
        # the edge case is when nothing fits our conditions
        # at all and shouldn't be reached
        return -1
